<script setup>
    ...
    const workoutIndices = computed(() => {
        return Object.keys(workoutProgram).map(key => parseInt(key));
    });
...
<template>
    <section id="grid">
        <button :disabled="props.firstIncompleteWorkoutIndex !== -1 && workoutIdx > props.firstIncompleteWorkoutIndex"
             @click="()=> props.handleSelectWorkout(workoutIdx)" :key="workoutIdx" 
            v-for="workoutIdx in workoutIndices" class="card-button plan-card">
            <div>
            ...
            <h3>{{workoutType[workoutIdx % 3]}}</h3>
        </button>
